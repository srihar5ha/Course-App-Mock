...
const authenticateJwt = (req, res, next) => {
  ...
  if (authHeader) {
    ...
    jwt.verify(token, secretKey, (err, user) => {
      ...
      req.user = user; //this user is not actually the whole user object, but only the payload. 
      //i.e  the above user is actually the payload returned by the callback in which we have 
      //{ username: ..., }. so we can access req.user.username in below methods.
      next();
    });
  } else {
    res.sendStatus(401);
